...
public class AutocadScriptRunner : IAutocadScriptRunner
{
    ...
    /// <inheritdoc />
    public async Task Run(Action<IAutocadScriptBuilder> action, bool showLog, CancellationToken cancellationToken)
    {
        var tempScriptFilePath = CreateScriptFile(action);

        var arguments = GetParams(tempScriptFilePath);
    ...
    private string CreateScriptFile(Action<IAutocadScriptBuilder> action)
    {
